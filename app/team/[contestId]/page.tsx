  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      <section className="surface-card rounded-[34px] p-8 md:p-10">
        ...
        <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-[var(--foreground)] md:text-5xl">
          확정한 아이디어 기준으로 팀 짜기 준비를 시작합니다.
        </h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--muted)]">
          이번 단계에서는 브레인스토밍 결과를 팀 빌딩 화면으로 안전하게 넘기는 진입점만 제공합니다. 실제 협업 대시보드와 AI 팀원 시뮬레이션은 다음 단계에서 연결합니다.
        </p>

        <div className="mt-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-[24px] border border-[var(--border)] bg-[rgba(255,255,255,0.03)] p-5">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">꿈</div>
            <p className="mt-3 text-sm leading-7 text-[var(--foreground)]">{handoff.why}</p>
          </div>
          <div className="rounded-[24px] border border-[var(--border)] bg-[rgba(255,255,255,0.03)] p-5">
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">추천 방향</div>
        ...
        <div className="mt-6 rounded-[28px] border border-[var(--border)] bg-[var(--surface-muted)] p-6">
          <div className="text-sm font-semibold text-[var(--foreground)]">AI가 이 아이디어를 1순위로 본 이유</div>
